...
class GaussianProcess(object):
    ...
    def __init__(self, xtrain, ytrain, param=None, noise=0.0):
        ...
        self.ytrain = ytrain
        self.param = (
            0.1 * np.ones(xtrain.shape[1]) if param is None else np.asarray(param)
        )
    ...
    def predict(self, xtest, param=None):
        ...
        var = np.linalg.cholesky(
            K_ss + 1e-5 * np.eye(xtest.shape[0]) - np.dot(Lk.T, Lk)
        )
        ...
        # P value
        logP = -0.5 * np.dot(z.T, z).reshape(1) - np.trace(
            np.log(np.abs(L) + 1e-10)
        ).reshape(1)
    ...
    def crossvalidated(self, p0=None, kfold=4, **args):
        ...
        for k in range(kfold):
            ...
            logging.info(
                "-> kfold: {}/{}, train/test: {}/{}, RMSE: {:.5}".format(
                    k + 1, kfold, self.xtrain.shape[0], xtest.shape[0], rms
                )
            )
    ...
    def tuneParameters(
        ...
        verbose : bool, optional
            Description
        iterLearnRateReduce : int, optional
            Description
        """
        ...
        for i in range(max_iters):
            ...
            if verbose:
                logging.info(
                    f"Iter: {i}, LR: {learn_rate}, logP: {y}, \
                      stepmax: {np.max(np.abs(step))}, params: {p}"
                )

            # Stopping conditions
            if np.all(np.abs(step) < eps):
                if verbose:
                    logging.info("No further improvement observed!")
                break
            elif i == max_iters - 1:
                if verbose:
                    logging.info("Maximum iteration number reached!")
...
if __name__ == "__main__":
    ...
    if True:
        ...
        [
            plt.plot(
                Xtest,
                YtestHat.reshape(-1, 1) + np.dot(Yvar, np.random.normal(size=(50, 1))),
            )
            for _ in range(10)
        ]

        plt.gca().fill_between(
            Xtest.flat, YtestHat - 3 * sigma, YtestHat + 3 * sigma, color="#dddddd"
        )
    ...
    if True:
        ...
        Xtrain = np.asarray([-2, -1]) + np.random.rand(18, 2) * (
            np.asarray([2, 3]) - np.asarray([-2, -1])
        )
        ...
        XtrainNew = np.asarray([-2, -1]) + np.random.rand(2, 2) * (
            np.asarray([2, 3]) - np.asarray([-2, -1])
        )
        ...
        ax.plot_wireframe(
            Xtest[:, 0].reshape(20, 20),
            Xtest[:, 1].reshape(20, 20),
            YtestHat.reshape(20, 20),
            color="r",
        )
        ax.plot_wireframe(
            Xtest[:, 0].reshape(20, 20),
            Xtest[:, 1].reshape(20, 20),
            YtestHat.reshape(20, 20) + 3 * sigma.reshape(20, 20),
            color="gray",
        )
        ax.plot_wireframe(
            Xtest[:, 0].reshape(20, 20),
