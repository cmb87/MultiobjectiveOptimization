...
def animateSwarm(Xcine, Ycine, contourfct, xbounds, store=False):
    ...
    for color, p in zip(colors, range(Xcine.shape[1])):
        lines["particleBody_{}".format(p)] = ax.plot(Xcine[0,p,0], Xcine[0,p,1], '-', color=color)[0]
        lines["particleHead_{}".format(p)] = ax.plot(Xcine[0,p,0], Xcine[0,p,1], '^', color=color)[0]


    def update(i, x, lines):
        lines["title"].set_text("Iter: {}".format(i+1))
        for p in range(x.shape[1]):
            lines["particleBody_{}".format(p)].set_data([x[i,p,0],x[i+1,p,0]], [x[i,p,1],x[i+1,p,1]])
            lines["particleHead_{}".format(p)].set_data(x[i+1,p,0], x[i+1,p,1])

    ani = animation.FuncAnimation(fig, update, (itermax-1), fargs=(Xcine, lines), interval=10*(itermax-1), blit=False)
    plt.xlabel("x1")
    plt.ylabel("x2")
    if store:
        ani.save(os.path.join("./","swarm_opti_{}.gif".format(1)), writer = 'mencoder', fps=15)
        plt.close()
    else:
        plt.show()
...
def animateSwarm2(Xcine, Ycine, xbounds, ybounds, store=False):
    ...
    for color, p in zip(colors, range(Xcine.shape[1])):
        lines["particleBody_{}".format(p)] = ax.plot(Ycine[0,p,0], Ycine[0,p,1], '-', color=color)[0]
        lines["particleHead_{}".format(p)] = ax.plot(Ycine[0,p,0], Ycine[0,p,1], '^', color=color)[0]
    ...
    ani = animation.FuncAnimation(fig, update, (itermax-1), fargs=(Ycine, lines), interval=10*(itermax-1), blit=False)
    #ani.save(os.path.join(self.dir,"modal_result_mode_{}.gif".format(mid)), writer = 'mencoder', fps=15)
    #plt.close()
    plt.xlabel("F1(x)")
    plt.ylabel("F2(x)")
    plt.grid(True)
