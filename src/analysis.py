...
class Analysis(object):
    ...
    def cull(pts, dominates):
        ...
        while remaining:
            ...
            for other in remaining[1:]:
                [new_remaining, dominated][Analysis.dominates(candidate, other)].append(other)
            if not any(Analysis.dominates(other, candidate) for other in new_remaining):
    ...
    def computeParetoOptimalMember(X, Y, targetdirection=None):
        tdir = np.asarray(X.shape[1]*[-1]) if targetdirection is None else np.asarray(targetdirection)
        assert len(tdir) == X.shape[1], "Target direction must match feature count"

        Y = tdir*Y.copy()
        Ypareto, Ydominated = Analysis.cull(Y.tolist(), Analysis.dominates)
    ...
    def computeParetoRanks(X, Y, targetdirection=None):
        ...
        while Xunranked.shape[0]>1:
            Xpareto, Ypareto, paretoIndex, Xunranked, Yunranked, _ = Analysis.computeParetoOptimalMember(Xunranked, Yunranked, targetdirection=targetdirection)
